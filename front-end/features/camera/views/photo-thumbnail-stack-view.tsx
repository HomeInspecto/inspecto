...
export default function PhotoThumbnailStackView({ photos, onPress }: PhotoThumbnailStackViewProps) {
  ...
  return (
    <Pressable
      onPress={onPress}
      style={{ position: 'relative', width: thumbnailWidth, height: thumbnailHeight }}
    >
      {photos.length > 1 &&
        photos.slice(-photosToShow - 1, -1).map((photo, index) => {
      ...
      >
        ...
        <View
          style={{
            position: 'absolute',
            top: (thumbnailHeight - 40) / 2,
            left: (thumbnailWidth - 40) / 2,
            pointerEvents: 'none',
          }}
        >
          <IconButton
            size="sm"
            color="primary"
            iconSlot={<Icon name={'pencil.tip'} size={24} color={COLORS.label.onLight.primary} />}
          />
        </View>
      </View>
    </Pressable>
